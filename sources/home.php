        ...
        foreach ($sp_baiviet as $rows) {
            ...
            <ul>
                <a href="<?= GET_link($full_url, $rows['seo_name']) ?>">
                    <li><img src="<?= checkImage($fullpath, $rows['icon'], $rows['duongdantin'], 'thumb_') ?>"
...
<div class="gioithieu_home_box"
     style="background: url(<?= $fullpath . "/" . $r['duongdantin'] . "/" . $r['icon'] ?>) center center no-repeat fixed; background-size: cover; -webkit-background-size: cover; -moz-background-size: cover; -o-background-size: cover; -ms-background-size: cover;">
    <div class="pagewrap">
        <div class="gioithieu_id_box">
            <li><img src="<?= checkImage($fullpath, $noidung['icon'], "datafiles/setone") ?>"
                     alt="<?= SHOW_text($noidung['tenbaiviet_' . $lang]) ?>"/>>
            </li>
            <ul>
                ...
                <h4>
                    <a href="<?= GET_link($full_url, SHOW_text($noidung['lien_ket'])) ?>"><?= $glo_lang['xem_chi_tiet'] ?>
                        ›</a></h4>
                <div class="clr"></div>
            </ul>
            <div class="clr"></div>
        ...
        <div class="tintuc_home_id">
            <div id="owl-spnb" class="owl-carousel owl-theme flex">
                ...
                foreach ($sp_baiviet as $rows) {
                    ?>
                    <ul>
                        <li><a href="<?= GET_link($full_url, $rows['seo_name']) ?>"><img
                        ...
                        <h3>
                            <a href="<?= GET_link($full_url, $rows['seo_name']) ?>"><?= SHOW_text($rows['tenbaiviet_' . $lang]) ?></a>
                        </h3>
