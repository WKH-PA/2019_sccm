...
function GUI_noidung_popup(){
  ...
  if($(".noidung_popup").val() == ''){
    ...
    $.ajax({
        type: "POST",
        url: "",
        data: {"id_send": $(".id_send").val(),"key_send": $(".key_send").val(), "nd_guimail": $(".nd_guimail").val(), "noidung_popup": $(".noidung_popup").val(), "cap_popup":$('#cap_popup').val(), "action_ajax": "send_popup"},
...
function DANGKY_email(url) {
  ...
  var v_name  = $("#ip_sentmail_name").val();
    $.ajax({
        ...
        data: {"v_email": v_email,"v_name": v_name, "capcha_hd":$('#capcha_hd').val(), "action_ajax": "dang-ky-email"},
        success: function(data)
        {
          try {
  ...
  if(!CHECK_phone("#s_dienthoai")){
    ...
    if(check_guisodt == 0){
      ...
      $.ajax({
        ...
        data: {"v_phone": v_phone,"v_name": v_name, "capcha_hd": $('#capcha_hdphone').val(), "action_ajax": "dang-ky-phone"},
        success: function(data)
        {
          $(".ajax_img_loading").hide();
          try {
...
function LOAD_ajax_product(url, id, step, key, total,numview, id_run = 0) {
  ...
  .always(function()
  {
      ...
      setTimeout(function(){GOTO_sport(".ajax_scron_"+page); }, 700);
      is_busy = false;
  });
  return false;
}
...
function PLAY_video(id){
  ...
  setTimeout(function(){GOTO_sport(".id_hide_video"); }, 200);
};
