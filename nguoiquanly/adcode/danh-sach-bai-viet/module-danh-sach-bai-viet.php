<?php
  if(isset($_GET['upload']) && (isset($_GET['edit']) && is_numeric($_GET['edit']))){
    ...
    {
      $sql_se     = DB_que("SELECT * FROM `#_baiviet_diendan` WHERE `id`='".$_GET['del_bl']."' LIMIT 1");

      if(mysql_num_rows($sql_se) > 0)
      {
        DB_que("DELETE FROM `#_baiviet_diendan` WHERE `id` = '".$_GET['del_bl']."'");
        // End xóa ảnh con        
    ...
    if(isset($_GET['del']))
    {
      ...
      if(mysql_num_rows($sql_se) > 0)
      {
        ...
        DB_que("DELETE FROM `#_baiviet_select_tinhnang` WHERE `id_baiviet` = '".$_GET['catalogid']."'");
        DB_que("DELETE FROM `#_baiviet_diendan` WHERE `id_baiviet` = '".$_GET['catalogid']."'");
        // Xóa ảnh con
    ...
    if(isset($_REQUEST['addgiatri']) AND isset($_REQUEST['maxvalu']))
    {
      ...
      for($i=0;$i<$_REQUEST['maxvalu'];$i++)
      {
        ...
        if(isset($_POST["xoa_gr_arr_$i"])){
          ...
          if(mysql_num_rows($sql_se) > 0)
          {
            ...
            DB_que("DELETE FROM `#_baiviet_select_tinhnang` WHERE `id_baiviet` = '".$idofme."'");
            DB_que("DELETE FROM `#_baiviet_diendan` WHERE `id_baiviet` = '".$idofme."'");
            // Xóa ảnh con
      ...
      if(isset($_FILES['is_muti_file'])) {
        ...
        foreach($_FILES['is_muti_file']['name'] as $name => $value) {
          ...
          DB_que("UPDATE  `$table` SET `icon` = '$img_real_name' WHERE `id` = '".$id_x."' LIMIT 1");
    ...
    if($s_chude != 0 ){
      if($id_step == 2) { 
        ...
        $mo              .= ' AND FIND_IN_SET( '.$s_chude.', `id_parent_muti`) <> 0 ';
      }
      else{
        $lay_all_kietxuat = LAYDANHSACH_idkietxuat($s_chude, $step);
    ...
    $sql     = DB_que("SELECT * FROM `$table` WHERE (`step`='".$step."' OR FIND_IN_SET(".$step.", `detail_cn`) > 0 ) $mo ORDER BY `catasort` DESC, `id` DESC LIMIT $pzz,$numview");
...
<script>
  function LOAD_sort(){
    ...
    $(".table-danhsach tr").each(function(index){
      $('td:nth-child(2) input[type="text"]',this).val(n_sort - index);
