<?php
  if(isset($_GET['them-moi']) || (isset($_GET['edit']) && is_numeric($_GET['edit']))){
    ...
    if(isset($_POST['tien_tich_luy']) && is_numeric($_POST['tien_tich_luy'])) {
        $tien = str_replace(",", "", $_POST['tien_tich_luy']);
        $tien = str_replace(".", "", $tien);
        DB_que("UPDATE `#_seo` SET `tien_tich_luy` = '".$tien."' LIMIT 1");
    }
    $chietkhau = DB_que("SELECT * FROM `#_seo` LIMIT 1");
    $chietkhau = mysql_fetch_assoc($chietkhau);

    if(isset($_GET['doi-diem'])) {
     
        $check = DB_que("SELECT * FROM `#_members` WHERE `id` = '".$_GET['doi-diem']."' LIMIT 1");
        $check = mysql_fetch_assoc($check);

        if($chietkhau['tien_tich_luy'] != 0) {
            $diem = (int)($check['so_tien_thanh_toan'] / $chietkhau['tien_tich_luy']);
            $tien_con_lai = $check['so_tien_thanh_toan'] - $diem * $chietkhau['tien_tich_luy'];
            DB_que("UPDATE `#_members` SET `so_tien_thanh_toan` = '$tien_con_lai', `diem_tich_luy` = `diem_tich_luy` + '$diem' WHERE `id` = '".$_GET['doi-diem']."' AND `phanquyen` = 0 LIMIT 1 ");
        }
    ...
    if(isset($_REQUEST['addgiatri']) AND isset($_REQUEST['maxvalu']))
    {
        ...
        {
            $idofme     = $_POST["idme$i"];            $cong_ty    = $_POST["cong_ty$i"];
            ...
            <section class="col-lg-12">
                ...
                <div class="box">
                    ...
                    <!-- <div class="form-group" style="margin: 0 10px;">
                        <p style="font-size: 13px; font-weight: 600; margin: 0 0 7px;">Số tiền / 1 điểm tích lũy</p>
                        <input type="text" class="form-control" name="tien_tich_luy" value="<?=number_format($chietkhau['tien_tich_luy']) ?>">
                    </div> -->
