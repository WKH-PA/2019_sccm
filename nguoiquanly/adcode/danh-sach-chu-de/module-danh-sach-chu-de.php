<?php
  if(isset($_GET['upload']) && (isset($_GET['edit']) && is_numeric($_GET['edit']))){
    ...
    $sql     = DB_que("SELECT * FROM `$table` WHERE `step`='".$step."'  AND `id_parent` = 0  ORDER BY `catasort` DESC LIMIT $pzz,$numview");
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td>
                              ...
                              <?=LAY_chude($id_parent, $step, 'id_parent SlectBox'.$cl, 'form-control', 0, $id_step, $ida, 'true', 0) ?>
                              <?php } ?>
                              <?php if(isset($_SESSION['admin'])){ ?>
                              <label>
